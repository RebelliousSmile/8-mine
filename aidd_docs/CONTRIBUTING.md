...
Generator skills consume the templates inside their `assets/` folder and write the output to the correct location for your AI tool.
...
| `aidd-vcs:02:pull-request/assets/`                 | Pull/merge request body template                         |

## Syncing Across Tools

The memory bank is shared automatically via the `<aidd_project_memory>` block kept in sync by `aidd-context:02:project-init`. Skills are loaded per-plugin by the runtime.

## Recommended Workflow

- Open a pull request for any new skill, agent, rule, or template.
...
- Stay within 5 to 10 percent deviation from a template structure.
